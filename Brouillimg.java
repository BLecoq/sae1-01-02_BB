...
public class Brouillimg {
    ...
    public static BufferedImage scrambleLines(BufferedImage inputImg, int[] perm) {
        int largeur = inputImg.getWidth();
        int hauteur = inputImg.getHeight();
        if (perm.length != hauteur) {
        ...
        for (int y = 0; y < hauteur; y++) {
            int yDestination = perm[y];
            for (int x = 0; x < largeur; x++) {
                out.setRGB(x, yDestination, inputImg.getRGB(x, y));
            }
        }
        return out;
    }
    ...
    /**
     * Recherche la meilleure clé par force brute selon le critère Pearson ou
     * Euclidien ou Variation Totale
     * @param gl image en niveaux de gris
    ...
    public static double[][] breakKey(int[][] gl, String type) {
        ...
        for (int k = 0; k < 32768; k++) {
            ...
            double score
                    = type.equals("pearson") ? scorePearson(gl, perm)
                    : type.equals("tv") ? scoreTV(gl, perm)
                    : scoreEuclidian(gl, perm);
            for (int i = 0; i < 3; i++) {
                boolean better
                        = type.equals("pearson") ? score > top3[i][0]
                        : score < top3[i][0];
                if (better) {
                    for (int j = 2; j > i; j--) {
                        top3[j][0] = top3[j - 1][0];
    ...
    /**
     * Calcule la distance de variation totale entre deux lignes.
     * @param gl image en niveaux de gris
     * @param ligne1 indice de la première ligne
     * @param ligne2 indice de la seconde ligne
     * @return distance TV
    ...
    /**
     * Calcule le score total de variation totale pour une image selon une
     * permutation.
     * @param gl image en niveaux de gris
     * @param permutation permutation des lignes
     * @return score TV
