...
public class Brouillimg {
    ...
    public static void main(String[] args) throws IOException {
        ...
        else if (args[3].equals("unscramble")) {
    ...
    public static int scrambledId(int id, int size, int key) {
        ...
        long indiceBrouille = ((r + (2 * s + 1) * id) % size);
    ...
    public static BufferedImage unScrambleLines(BufferedImage inputImg, int[] perm) {
        ...
        for (int xSource = 0; xSource < height; xSource++) {
            int xDestination = perm[xSource];
            for (int y = 0; y < width; y++) {
                int pixel = inputImg.getRGB(y, xSource);
                out.setRGB(y, xDestination, pixel);
            }
        }
        return out;
    }
